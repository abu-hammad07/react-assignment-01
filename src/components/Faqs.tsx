...
const faqsData = [
    ...
    },
    {
        ...
        answer:
            "Generally, it is accepted to use Windster in open-source projects, as long as it is not a UI library, a theme, a template, a page-builder that would be considered as an alternative to Windster itself. With that being said, feel free to use this design kit for your open-source projects.",
    },
];
                    ...
                    <div className="grid border-t border-gray-200 pt-8 text-left md:grid-cols-2 md:gap-16 dark:border-gray-700">
                        <div>
                            {faqsData.slice(0, 4).map((faq, index) => (
                                <FaqsItems key={index} question={faq.question} answer={faq.answer} />
                            ))}
                        </div>
                        <div>
                            {faqsData.slice(4).map((faq, index) => (
                                <FaqsItems key={index + 4} question={faq.question} answer={faq.answer} />
                            ))}
                        </div>
                    </div>
                </div>
